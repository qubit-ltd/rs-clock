...
impl ManualWaiterRegistry {
    ...
    pub(crate) fn register_timer(&mut self, deadline: Duration) -> u64 {
        ...
        assert!(inserted.is_ok(), "manual timer waiter identifier must be unique",);
        waiter_id
    }
    ...
    pub(crate) fn unregister_timer(&mut self, waiter_id: u64) -> Option<Option<Waker>> {
        self.timer_waiters.remove(&waiter_id).map(|entry| entry.into_value())
    }
    ...
    pub(crate) fn register_observer(&mut self, expected_count: usize, count: usize) -> Option<u64> {
        ...
        self.count_observers.insert(observer_id, (expected_count, None));
        Some(observer_id)
    }
    ...
    pub(crate) fn poll_observer(&mut self, observer_id: u64, context: &Context<'_>) -> (Poll<()>, Option<Waker>) {
    ...
    pub(crate) fn poll_timer(
        ...
        let Some(deadline) = self.timer_waiters.get_entry(&waiter_id).map(|entry| *entry.order()) else {
