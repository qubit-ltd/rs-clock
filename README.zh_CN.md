...
为 Rust 提供可注入的 wall clock、monotonic clock，以及可确定性测试的阻塞和异步
sleeper。
...
| 阻塞等待 | `BlockingSleeper` | `StdBlockingSleeper` | `ManualBlockingSleeper` |
| 异步等待 | `AsyncSleeper` | `TokioAsyncSleeper` | `ManualAsyncSleeper` |

Wall time 可能跳变，适合表示对外有意义的时间戳。一个 clock domain 内的 monotonic
time 永不倒退，适合测量耗时、实现 retry 和 timeout。每个 sleeper 都持有并暴露与
自己配对的 monotonic clock，从而避免 deadline 计算静默使用另一条 timeline。
...
需要 Tokio clock 和 sleeper 类型时启用对应 feature：
...
Manual async sleep 和 manual 协调与 runtime 无关，不要求启用本库的 `tokio`
feature。
...
clock
    ...
    .expect("有效 sleep 应存在未来 deadline");
task.await??;
Ok(())
}
```
...
wall reanchor、subscription、trait object 注入和错误处理。
