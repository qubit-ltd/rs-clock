...
#[cfg(loom)]
#[test]
fn test_std_timer_waiter_model_poll_racing_with_complete() {
    loom::model(|| {
        let waiter = loom::sync::Arc::new(
            modeled_std_timer_waiter::StdTimerWaiter::new(),
        );
...
#[cfg(loom)]
#[test]
fn test_std_timer_waiter_model_poll_racing_with_worker_failure() {
    loom::model(|| {
        let waiter = loom::sync::Arc::new(
            modeled_std_timer_waiter::StdTimerWaiter::new(),
        );
