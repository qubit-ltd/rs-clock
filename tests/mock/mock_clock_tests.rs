...
/// Verifies positive nanosecond overflow is clamped to chrono and millis bounds.
///
/// # Errors
...
fn test_time_and_millis_clamp_positive_overflow() {
    ...
    let clock = MockClock::with_timeline(DateTime::<Utc>::MAX_UTC, timeline.clone());
...
/// Verifies negative nanosecond overflow is clamped to chrono and millis bounds.
///
/// # Errors
...
/// Verifies out-of-range negative chrono values clamp through the fallback path.
///
/// # Errors
/// The test fails if values below chrono's minimum no longer clamp to `MIN_UTC`.
#[test]
