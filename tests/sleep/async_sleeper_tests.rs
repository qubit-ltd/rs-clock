...
async fn test_mock_sleep_for_async_zero_duration_completes_immediately() {
    ...
    sleeper
        .timeline()
        .reset()
        .expect("completed zero-duration sleep should not register an active waiter");
}
...
async fn test_mock_sleep_for_async_registers_active_waiter_until_completion() {
    ...
    assert!(
        timeline.wait_for_blocked_waiters(MockWaiterKind::Sleep, 1, Duration::from_millis(20)),
